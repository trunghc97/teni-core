...
1. Put audio in `assets/stories/`, `assets/music/`, `assets/sfx/`, or `assets/responses/`.
2. Add or edit JSON object files in `knowledge/` and response templates in
   `assets/responses/templates.json`.
3. Run `pio run -e teni_esp32s3`. PlatformIO automatically regenerates `assets.h`, `assets.cpp`,
   `build/asset_index.json`, and `build/flash_report.json`.
...
[hardware wiring](docs/HARDWARE_WIRING.md), and [MVP1 migration notes](docs/MIGRATION_MVP1_TO_MVP1_1.md).
